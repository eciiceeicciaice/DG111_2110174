...
int loadScores(const char *filename, Entry list[])
{
    ...
    {
        if (sscanf(line, "%29[^,],%d", list[count].name,
                   &list[count].score) == 2)
        {
            count++;
        }
...
// Insert into sorted list (descending). Return 1 if the entrymade top - 5
...
int main()
{
    ...
    // 3. Keep asking for new scores until the user types "q" asthe name
    char name[NAME_LEN];
    int score;
    while (1)
    {
